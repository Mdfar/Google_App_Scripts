/**
 * Main function to process health data and generate a report of lost clients.
 */
function processHealthLostClients() {
    try {
      ...
      var validRecords = healthData.filter(record => !clientStatuses.includes(record[3]));
      ...
      Logger.log('Error in processHealthLostClients: ' + error.message);
    }
  }
  ...
  function filterRecordsByYear(records, year, timezone) {
    return records.filter(record => {
      var recordYear = Utilities.formatDate(new Date(record[7]), timezone, "yyyy");
  ...
  function filterRecordsByMonth(records, year, month, timezone) {
    return records.filter(record => {
      var recordDate = new Date(record[7]);
