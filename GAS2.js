/**
 * Main function to fetch health data from API and update the spreadsheet
 */
function fetchAndStoreHealthData() {
    try {
      ...
      Logger.log('Error in fetchAndStoreHealthData: ' + error.message);
    }
  }
  ...
  function processContactEntry(contact, timezone) {
    ...
    customFields.forEach(field => {
      ...
      if ([136, 138, 141, 158, 933, 1102].includes(fieldId)) {
    ...
    if (!entry[2]) {
      ...
      var dateField = new Date(entry[7]);
      entry[7] = Utilities.formatDate(dateField, timezone, "yyyy-MM-dd");
      return entry;
    }
  }
